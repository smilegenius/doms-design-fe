...
import { ArrowRight, Mail, ShieldCheck, Users, Inbox, ArrowLeft } from 'lucide-react';

// Small black wordmark logo (same SVG as LoginPage / PortalSelect — namespaced gradient id).
const SmileGeniusLogo = () => (
  <svg width="171" height="40" viewBox="0 0 171 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-36 h-auto">
    <path d="M165.562 29.1068C166.989 26.0695 164.967 27.0625 163.778 27.9386C144.154 42.3949 126.639 31.467 123.506 29.8808C121.977 29.1068 121.977 29.1068 121.977 29.1068C125.035 34.0716 131.977 39.3547 143.129 39.3547C155.204 39.3547 163.778 32.9034 165.562 29.1068Z" fill="url(#invite_grad)"/>
    ...
    <defs>
      <linearGradient id="invite_grad" x1="166.052" y1="33.2046" x2="121.977" y2="33.2046" gradientUnits="userSpaceOnUse">
...
// Two-step entry for a new dental-group admin clicking through from their invite:
//   step "email"  → realistic email body preview (what lands in their inbox).
//                   "Start Setup" inside the email opens step "setup".
//   step "setup"  → branded "Welcome to Smile Genius" landing card with the live
//                   Start Setup CTA that drops them into the 3-step onboarding wizard.
type Step = 'email' | 'setup';
...
export default function InvitePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F7E2F8]/60 to-[#AEE3E6]/60 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-2xl">

        {/* ── STEP 1: Email body preview ─────────────────────────────── */}
        {step === 'email' && (
          <>
            <div className="bg-white rounded-2xl shadow-lg border border-[#E0E0E6] overflow-hidden reveal">
              {/* Mail-client chrome */}
              <div className="bg-[#F8F9FC] border-b border-[#F0EFF6] px-5 py-2.5 flex items-center gap-2">
                <Inbox className="w-3.5 h-3.5 text-[#717182]" />
                <span className="text-[10px] font-bold text-[#A0A0B0] uppercase tracking-widest">Email · Inbox preview</span>
              </div>

              {/* Sender row */}
              <div className="px-6 py-4 border-b border-[#F0EFF6]">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#4D8EF7] to-[#A59DFF] flex items-center justify-center flex-shrink-0 text-white text-xs font-bold">
                    SG
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-[#030213]">
                        Smile Genius <span className="text-[#A0A0B0] font-normal">&lt;invites@smilegenius.co.uk&gt;</span>
                      </p>
                      <span className="text-[10px] text-[#A0A0B0] flex-shrink-0">just now</span>
                    </div>
                    <p className="text-[11px] text-[#717182] mt-0.5">to <span className="text-[#5A5568]">you@dentalgroup.com</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-5">
                <p className="text-base font-semibold text-[#030213] mb-3">
                  Smile Genius invited you to set up your dental group portal
                </p>
                <div className="text-sm text-[#5A5568] leading-relaxed space-y-3">
                  <p>Hi there,</p>
                  <p>
                    You've been invited to set up the{' '}
                    <span className="font-semibold text-[#030213]">Smile Genius portal</span>{' '}
                    for your dental group. The portal puts your practices, staff, suppliers, invoices and analytics in one place.
                  </p>
                  <p>Click the button below to get started — it takes about <span className="font-semibold text-[#030213]">3 minutes</span>.</p>
                </div>

                {/* Live CTA inside the email — clicking advances to step 2 */}
                <button
                  onClick={() => setStep('setup')}
                  className="mt-5 inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-gradient-to-r from-[#4D8EF7] to-[#A59DFF] text-white text-sm font-semibold hover:opacity-90 transition-opacity shadow-sm"
                >
                  Start Setup
                  <ArrowRight className="w-4 h-4" />
                </button>

                <p className="text-[11px] text-[#A0A0B0] mt-5 leading-relaxed">
                  This invitation expires in 7 days. If you didn't expect it, you can ignore this email.<br />
                  Need help? Reply to this email or contact{' '}
                  <span className="text-[#5A5568]">support@smilegenius.co.uk</span>.
                </p>
              </div>

              {/* Email footer */}
              <div className="px-6 py-3 bg-[#FAFBFC] border-t border-[#F0EFF6]">
                <p className="text-[10px] text-[#A0A0B0] leading-snug">
                  Smile Genius · Dental Operations Suite ·{' '}
                  <span className="text-[#5A5568]">smilegenius.co.uk</span>
                </p>
              </div>
            </div>

            <p className="text-center text-xs text-[#A0A0B0] mt-4">
              This is a preview of the invitation email a new admin receives. Click <span className="font-semibold text-[#5A5568]">Start Setup</span> above to continue the demo.
            </p>
          </>
        )}

        {/* ── STEP 2: Welcome / Start Setup landing card ───────────────── */}
        {step === 'setup' && (
          <>
            {/* Back to email body link */}
            <button
              onClick={() => setStep('email')}
              className="flex items-center gap-1.5 text-xs text-[#717182] hover:text-[#030213] transition-colors mb-3"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              Back to invitation email
            </button>

            <div className="bg-white rounded-3xl shadow-xl px-8 sm:px-10 py-10 reveal">
              {/* Header — Smile Genius wordmark + "You're invited" tag */}
              <div className="flex flex-col items-center text-center mb-8">
                <div className="mb-5">
                  <SmileGeniusLogo />
                </div>
                <p className="text-xs font-bold text-[#A59DFF] uppercase tracking-widest mb-2">You're invited</p>
                <h1 className="text-2xl sm:text-3xl font-bold text-[#030213] mb-2">
                  Welcome to Smile Genius
                </h1>
                <p className="text-sm text-[#717182] max-w-md leading-relaxed">
                  You've been invited to set up your dental group's portal — invoices, suppliers, staff and analytics in one place.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
                {[
                  { icon: ShieldCheck, title: 'Set your password', sub: 'Secure your account in seconds' },
                  { icon: Mail,        title: 'Add your details',  sub: 'Confirm your group profile' },
                  { icon: Users,       title: 'Invite your team',  sub: 'Bring colleagues onboard' },
                ].map((s) => {
                  const Icon = s.icon;
                  return (
                    <div key={s.title} className="bg-[#F8F9FC] border border-[#F0EFF6] rounded-2xl p-4 flex flex-col items-start gap-2">
                      <div className="w-9 h-9 rounded-xl bg-white border border-[#E0E0E6] flex items-center justify-center">
                        <Icon className="w-4 h-4 text-[#4D8EF7]" />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#030213] leading-tight">{s.title}</p>
                        <p className="text-[11px] text-[#717182] leading-snug mt-0.5">{s.sub}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              {/* Primary CTA → onboarding wizard */}
              <button
                onClick={() => navigate('/onboarding')}
                className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#4D8EF7] to-[#A59DFF] text-white text-sm font-semibold rounded-xl py-3.5 hover:opacity-90 transition-opacity"
              >
                Start Setup
                <ArrowRight className="w-4 h-4" />
              </button>

              <p className="text-center text-[11px] text-[#A0A0B0] mt-6">
                Already set up?{' '}
                <button onClick={() => navigate('/login')} className="text-[#4D8EF7] hover:underline font-medium">
                  Sign in instead
                </button>
              </p>
            </div>
