...
const SmileGeniusLogo = () => (
  <svg width="171" height="40" viewBox="0 0 171 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto">
    <path d="M165.562 29.1068C166.989 26.0695 164.967 27.0625 163.778 27.9386C144.154 42.3949 126.639 31.467 123.506 29.8808C121.977 29.1068 121.977 29.1068 121.977 29.1068C125.035 34.0716 131.977 39.3547 143.129 39.3547C155.204 39.3547 163.778 32.9034 165.562 29.1068Z" fill="url(#admin_grad)"/>
    ...
    <defs>
      <linearGradient id="admin_grad" x1="166.052" y1="33.2046" x2="121.977" y2="33.2046" gradientUnits="userSpaceOnUse">
...
export default function AdminLayout({ children, activePage = 'dashboard', onNavigate }: AdminLayoutProps) {
  const [internalActive, setInternalActive] = useState(activePage);
  const active = onNavigate ? activePage : internalActive;
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FC]">
      ...
      <button
        ...
        className="lg:hidden fixed top-3 left-3 z-40 p-2 bg-white border border-[#E0E0E6] rounded-lg text-[#717182] hover:text-[#030213] hover:bg-[#F8F9FC] transition-colors shadow-sm"
        title="Menu"
      >
      ...
      <aside
        className={`flex flex-col bg-white transition-transform duration-200 ease-in-out
          fixed inset-y-0 left-0 z-50 lg:static lg:inset-auto lg:flex-shrink-0 w-[240px] lg:w-[200px]
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="p-6 pb-8">
            ...
            <div className="mt-3 flex items-center gap-1.5 px-1">
              <span className="text-[10px] font-semibold text-[#A59DFF] uppercase tracking-wider">Super Admin</span>
            </div>
          </div>
            ...
            {navItems.map(({ id, label, icon: Icon }) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 mb-1 rounded-lg text-left transition-colors relative ${
                    isActive ? 'bg-white/30 text-[#030213]' : 'text-[#5A5568] hover:bg-white/20'
                  }`}
                >
                  {isActive && (
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-r from-[#4D8EF7] to-[#A59DFF] rounded-r" />
                  )}
          ...
          <div className="p-4 border-t border-[#D4CEE1] space-y-0.5">
            {/* Settings + Logout */}
            <div className="flex items-center rounded-lg">
              <button
                onClick={() => handleNav('settings')}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-left transition-colors relative flex-1 ${
                  active === 'settings' ? 'bg-white/30 text-[#030213]' : 'text-[#5A5568] hover:bg-white/20'
                }`}
              >
                {active === 'settings' && (
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-r from-[#4D8EF7] to-[#A59DFF] rounded-r" />
                )}
                <FilledGear className="w-5 h-5" />
                <span className="text-sm font-medium">Settings</span>
              </button>
              <button
                onClick={() => setShowLogoutConfirm(true)}
                title="Log out"
                className="p-2 mr-1 text-[#5A5568] hover:text-[#C62828] hover:bg-white/30 rounded-lg transition-colors flex-shrink-0"
              >
      ...
      <main className="flex-1 overflow-auto relative pt-12 lg:pt-0">
        {children}

        {/* Floating Switch Portal */}
        <button
          onClick={() => navigate('/portal-select')}
          title="Switch portal"
          className="fixed bottom-5 right-5 group flex items-center gap-2 bg-white border border-[#E0E0E6] rounded-full shadow-md px-3 py-2 text-[#5A5568] opacity-40 hover:opacity-100 transition-all hover:shadow-lg hover:border-[#4D8EF7]/40 z-40"
        >
          <ChevronRight className="w-4 h-4 rotate-180 flex-shrink-0" />
          <span className="text-xs font-medium max-w-0 overflow-hidden group-hover:max-w-[80px] transition-all duration-200 whitespace-nowrap">Switch portal</span>
        </button>
      </main>
